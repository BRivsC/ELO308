...
timestamps = [fila[0] for fila in datos]  # Ojo: timestamp no registra el tiempo correcto! Generar vector de tiempo usando la frecuencia de muestreo!!!
...
# Fix de timestamps para generar vectores de tiempo correctamente
if sample_rate == "1000":
    timestamps_fix = np.array(range(0,len(CH1)))
else:
    timestamps_fix = np.array(range(0,len(CH1)))*2


# Graficar CH1 y la referencia
#axs[0].plot(timestamps, CH1, label='CH1', color='blue')
axs[0].plot(timestamps_fix, CH1, label='CH1', color='blue')
...
axs[2].plot(timestamps_fix, CH3, label='CH3', color='red')
